...
(define (distance city-a city-b)
  (let ((city-a-lat (get-lat city-a))
        (city-a-lon (get-lon city-a))
        (city-b-lat (get-lat city-b))
        (city-b-lon (get-lon city-b)))
    (sqrt (+ (expt (- city-a-lat city-b-lat) 2)
             (expt (- city-a-lon city-b-lon) 2)))))

(define (closer-city lat lon city-a city-b)
  (let ((temp-city (make-city 'temp lat lon)))
    (let ((dis-to-a (distance temp-city city-a))
          (dis-to-b (distance temp-city city-b)))
      (if (< dis-to-a dis-to-b) (get-name city-a) (get-name city-b)))))
...
(define (student-get-classes student) (cdr student))

(define (teacher-get-name teacher) (car teacher))

(define (teacher-get-class teacher) (car (cdr teacher)))

(define (teacher-get-students teacher) (cdr (cdr teacher)))
...
(define (teacher-hold-class teacher)
  (let ((teacher-name (teacher-get-name teacher))
    ...
    (teacher-create teacher-name
                    teacher-class
                    (map (lambda (student) (student-attend-class student teacher-class)) 
