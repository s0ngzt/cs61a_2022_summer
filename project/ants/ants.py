...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur_place, range_counter = self.place, 0
        while cur_place is not None:
            if self.min_range <= range_counter <= self.max_range:
                if len(cur_place.bees) > 0 and type(cur_place) is not Hive:
                    return random.choice(cur_place.bees)
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            ...
            if len(self.place.bees) > 0:
                for bee in self.place.bees[:]:
                    Bee.reduce_health(bee, self.damage)
    ...
    def action(self, gamestate):
        if self.chew_timer > 0:
            ...
            if self.place:
                ...
                unlucky_bee = random.choice(bees) if bees else None
                if unlucky_bee:
                    Bee.reduce_health(unlucky_bee, unlucky_bee.health)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.queen_ant_created:
            gamestate.queen_ant_created = True
            ...
            while begin:
                if begin.ant:
                    ...
                    if hasattr(begin.ant, "ant_contained") and begin.ant.ant_contained:
                        begin.ant.ant_contained.double()
                begin = begin.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            ...
            Insect.reduce_health(insect, damage)
            if damage:
                self.insects_shot += 1
