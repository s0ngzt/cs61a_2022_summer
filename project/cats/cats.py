...
def about(topic):
    ...
    def select_func(paragraph: str):
        return any(x in remove_punctuation(paragraph.lower()).split() for x in topic)
...
def autocorrect(
    ...
    if typed_word in word_list:
        return typed_word
    res = min(word_list, key=lambda w: diff_function(typed_word, w, limit))
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    player_num = len(match["times"])
    rows = zip(*match["times"])
    ...
    for idx, num in enumerate(each_row_shortest_time):
        ans[num].append(word_at(match, idx))
