...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    pig_out = False
    all_dices = [dice() for _ in range(num_rolls)]
    for d in all_dices:
        if d == 1:
            pig_out = True
...
def digit_fn(digit):
    ...
    def f0(value): return value + 1
    def f1(value): return value ** 2
    def f2(value): return value * 3
    def f3(value): return value // 4
    def f4(value): return value - 5
    def f5(value): return value % 6
    def f6(value): return int((value % 7) * 8)
    def f7(value): return int(value * 8.8)
    def f8(value): return int(value / 99 * 15) + 10
    def f9(value): return value
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            num_player0_rolls = strategy0(score0, score1)
            score0 = score0 + \
                take_turn(num_player0_rolls, score0, score1, dice, goal)
            score0 = score0 + hog_pile(score0, score1)
        else:
            num_player1_rolls = strategy1(score1, score0)
            score1 = score1 + \
                take_turn(num_player1_rolls, score1, score0, dice, goal)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if cur_leader == None:
        return None, None
    if not (cur_leader == last_leader):
        message = "Player {} takes the lead by {}".format(
            cur_leader, abs(score0-score1))
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(hefty_hogs(score, opponent_score) + score, opponent_score) > 0:
        return 0
